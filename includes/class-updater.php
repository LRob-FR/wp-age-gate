 ...
 * release zip must be attached as an asset named "lrob-age-gate-X.Y.Z.zip"
 ...
 * of updating).
 *
 * The API base is derived from LROB_AGEGATE_REPO_URL, so moving the repo to
 * another host/owner only means editing that one constant.
 */

if (!defined('ABSPATH')) exit;

class LRob_AgeGate_Updater {

    // Back-off when the server is unreachable, so admin pages don't each pay a
    // connection timeout.
    const TRANSIENT_KEY      = 'lrob_agegate_release_fail';
    const TRANSIENT_TTL_FAIL = 5 * MINUTE_IN_SECONDS;
    const PLUGIN_SLUG        = 'lrob-age-gate';
    ...
    public function check_for_update($transient) {
        ...
        if (version_compare(LROB_AGEGATE_VERSION, $remote_version, '>=')) {
            return $transient;
        }
        ...
        $update = (object) array(
            ...
            'plugin'       => LROB_AGEGATE_BASENAME,
            'new_version'  => $remote_version,
            'url'          => LROB_AGEGATE_REPO_URL,
            'package'      => $zip_url,
        ...
        $transient->response[LROB_AGEGATE_BASENAME] = $update;
        return $transient;
    }
    ...
    public function plugin_info($result, $action, $args) {
        ...
        return (object) array(
            ...
            'homepage'      => LROB_AGEGATE_REPO_URL,
            'requires'      => '6.8',
            'requires_php'  => '8.2',
            'tested'        => $this->tested_wp_version(),
            ...
            'sections'      => array(
                'description' => __('Age verification gate with configurable presets for alcohol, adult content, and age-restricted products.', 'lrob-age-gate'),
    ...
    /**
     * https://git.lrob.net/WP/age-gate → https://git.lrob.net/api/v1/repos/WP/age-gate/releases/latest
    ...
    private function api_url() {
        $url = defined('LROB_AGEGATE_REPO_URL') ? LROB_AGEGATE_REPO_URL : '';
